...
<?php
                    include 'conn.php';
                    $no = 1;
				    $data = mysqli_query($conn,"select * FROM menu_martabak where id_category=7 AND no_varian='NON' AND stok='Tersedia';");
...
<?php
                    include 'conn.php';
                    $no = 1;
				    $data = mysqli_query($conn,"select * FROM menu_martabak where id_category=7 AND stok='Tidak Tersedia';");
				    while($d = mysqli_fetch_array($data)){
              $price=$d['price'];
                ?>
